...
void checkSuccess(int timelimit, ternary actiontype)
{                // Check objective sequence
  tiltCount = 0; // sets tilt count to 0

  for (int count = 0; count < timelimit; count++)
  { // Iter                   // Setting based on input
    if (checkInput(actiontype) == 1)
    {                 // Check if user performed action
      continu = true; // Proceed with game
      score++;        // Increment user's score
      return;
    }
    else if (checkInput(actiontype) == 2)
    { // Check if user performed wrong action
      break;
    }
    else
    {
...
int checkInput(ternary actiontype)
{                           // Check input signal(s)
  ...
  { // Tilt
    ...
    { // If any other action return false
      return 2;
    }
  ...
  { // Ring
    ...
    { // Check if joystick is moved far enough
      return 1;
    }
  ...
  { // Udder
    if (!digitalRead(udder))
    { // Check if udder is yanked
      return 1;
    }
    else if (digitalRead(tilt))
    { // If any other action return false
...
void startup()
{ // Startup sequence at beginning of game
...
void fail()
{                  // Ending sequence to finish game
